...
# Create the dictionaries that map each word to its number of sequences
...
# create two dictionaries that map the questions words and the answer words to a unique integer
...
#Creating the inverse dictionary of answersword2int dictionary
    
answersints2word={w_i:w for w,w_i in answersword2int.items() }
    
#Adding the End of String to the end of the every answer
for i in range(len(clean_answers)):
    clean_answers[i] += '<EOS>'
    
#Translating all the questions and the answers into integers
#And replacing all the words that were filtetred by <'OUT'>
...
#Decoding the trainning set
    
    
def decode_training_set(encoder_state,decoder_cell,decoder_embedded_input,sequence_length,decoding_scope,output_function,keep_prob,batch_size):
    attention_states=tf.zeros([batch_size,1,decoder_cell.output_size])
    attention_keys,attention_values,attention_score_function,attention_construct_function =tf.contrib.seq2seq(attention_states,attention_option='bahdanau', num_units=decoder_cell.output_size)
...
def decode_test_set(encoder_state,decoder_cell,decoder_embeddings_matrix,sos_id,eos_id,maximum_length,num_words,sequence_length,decoding_scope,output_function,keep_prob,batch_size):
    ...
    test_decoder_function=tf.contrib.seq2seq.attention_decoder_fn_inference(output_function,
                                                                            ...
                                                                            num_words,
                                                                            name="attn_dec_inf")
    test_predictions,decoder_output,decoder_final_state,decoder_final_context_state=tf.contrib.seq2seq.dynamic_rnn_decoder(decoder_cell,test_decoder_function,scope= decoding_scope)
...
def decoder_rnn(decoder_embedded_inputs,decoder_embeddings_matrix,encoder_state,num_words,sequence_length,rnn_size,num_layers,word2int,keep_prob,batch_size):
    with tf.variable_scope("decoding") as decoding_scope:
        ...
        output_function= lambda x: tf.contrib.layers.fully_connected(x,num_words,None,scope=decoding_scope,weights_intialiazer=weights,
                                                                     biases_intializer=biases)
        training_prediction=decode_training_set(encoder_state,decoder_cell,decoder_embedded_inputs,sequence_length,decoding_scope,output_function,keep_prob,batch_size)
        decoding_scope.reuse_variables()
        test_prediction=decode_test_set(encoder_state,decoder_cell,decoder_embeddings_matrix,
                                        word2int['<SOS>'],
                                        word2int['<EOS>'],sequence_length-1,num_words,decoding_scope,output_function,keep_prob,batch_size)
...
def seq2seq_model(inputs,targets,keep_prob,batch_size,sequence_length,answers_num_words,question_num_words,encoder_embedding_size,decoder_embedding_size,rnn_size,num_layers,questionswords2int):
    ...
    training_predictions,test_predictions=decoder_rnn(decoder_embedded_inputs,decoder_embeddings_matrix,encoder_state,question_num_words,sequence_length,rnn_size,num_layers,questionsword2int,keep_prob,batch_size)
...
with tf.name_scope("optimization"):
    ...
    optimizer_gradients_clipping= optimizer.apply_gradients(clipped_gradients)
    
#Padding the sequences with the <PAD> token
#Questions:['who','are','you']
#Answers:[<SOS>,'I','am','a','bot','.',<EOS>]
    
def apply_padding(batch_of_sequences,word2int):
    max_sequence_length=max([len(sequence)for sequences in batch_of_sequences])
    return[sequence +[word2int['<PAD>']]*(max_sequence_length-len(sequences)) for sequence in batch_of_sequences]
...
for epoch in range(1,epochs+1):
    for batch_index,(padded_questions_in_batch,padded_answers_in_batch)in enumerate(split_into_batches(training_questions,training_answers,batch_size)):
        ...
        _, batch_training_loss_error=session.run([optimizer_gradients_clipping,loss_error],
                                                 {inputs:padded_questions_in_batch,
                                                 targets:padded_targets_in_batch,lr:learning_rate,
        ...
        if batch_index% batch_index_validation_loss==0 and batch_index >0:
            ...
            if average_validation_loss_error<= min(list_validation_loss_error):
                ...
                print('Sorry  I need to practice more')
                early_stopping_check += 1
                if early_stopping_check == early_stopping_stop:
                     break
    if early_stopping_check == early_stopping_stop:
        print("My apolizes,I cannot speak anymore.This is the best can i do now")
        break
print("GameOver")
                

############ PART-4 TESTING THE SEQ2SEQMODEL##############
